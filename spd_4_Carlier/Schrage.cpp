...
Schrage::Schrage(const Schrage& orig) {
}
...
int Schrage::wczytaj(std::string nazwa_pliku) {
...
void Schrage::sortuj_r() {
    quicksort(0, ilosc_zadan - 1);
}

void Schrage::dodaj_q(int * dane) {
...
// wyznaczanie a
int Schrage::min_K(int max_K, int Cmax) {
    int min_K, tmp = 0;
...
// wyznaczanie c
int Schrage::c(int min_K, int max_K) {
    int tmp = -1;
    bool nie_podzialnalazl = true;
...
int Schrage::carlier(int gorne_ogr) {
    ...
    zadanie_c = c(poczatek_K, koniec_K); // c
    ...
    if (dolne_ogr < gorne_ogr) {
        gorne_ogr_tmp = carlier(gorne_ogr);
        // jezeli ok
...
//Quick Sort
void Schrage::zamien(int pierwszy, int drugi) {
    int * tmp;
...
int Schrage::podziel(int poczatek, int koniec, int pivot) {
...
void Schrage::quicksort(int lewy, int prawy) {
    if (prawy > lewy) {
